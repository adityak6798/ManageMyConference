...
default, a credential-gated live adapter verified in the 2026-08-13 staging smoke, and — unique to
this port — a draft-only guarantee enforced in storage rather than by convention (issue #110).
...
Signed webhooks consume the existing `event` delivery through a composed fan-out consumer. A partial consumer result is retryable, so every consumer is idempotent: schedule mail by `(event, publication version, speaker)` and webhooks by `(subscription, event record)`. Webhook deliveries have their own lease, immutable attempts, three-attempt policy, terminal state and history because an attacker-chosen HTTPS destination is not a communications provider. The Worker never fetches that URL directly: it sends an authenticated command to the separately operated trusted egress service. That service resolves every A and AAAA answer at subscription time and again immediately before send, rejects the complete set if any answer is not globally routable, pins one approved address to the target connection, refuses redirects, bounds the request, discards the target body, and returns only a normalized result/status. Its bearer token is addressed only to the enforcement origin and is never a target header. With all four bindings absent, webhook routes return `503 WEBHOOK_UNAVAILABLE` and scheduled fan-out is disabled while unrelated routes continue to run; partial configuration fails closed during composition. Receiver bodies never enter storage, logs, or error envelopes. Mutation idempotency binds one organization, operation, and normalized request hash to its original response for as long as webhook history is retained; a different operation/body under the key is refused. Current/previous HMAC keys and secret-bearing idempotency responses are AES-GCM envelopes with purpose-bound authenticated data and explicit wrapping-key version metadata; no raw key or response is stored.
