...
For `catch` blocks and Promise rejection callbacks, explicit handling means rethrowing or calling the approved ownership boundary (`logger.error`, `logger.warn`, `reportError`, UI `setError`, or a `useActionFeedback` handle's `announce("error", …)`). `announce` counts because it both renders the failure next to the control that caused it and publishes it to a live region, so the failure reaches sighted and assistive-technology users alike. Returning a fallback or calling an arbitrary recovery function is not evidence of handling; add the actual reporting/state transition or an adjacent `ERROR-INTENT:` rationale.
