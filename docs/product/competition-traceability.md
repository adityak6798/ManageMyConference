...
| 3 | templated speaker communications, and "calendar invites delivered directly to each speaker's own calendar (Gmail, Outlook, iCal)" (issues #52, #56) | **partial** — immutable templates, a durable outbox with retry/terminal/recovery, a conformant `.ics` **download**, and an organizer who can compose a template version and send it to the event's speakers from the console, each recipient getting their own delivery carrying the rendered message. Acceptance, task assignment, reviewer assignment, an accept/decline decision and a published schedule now enqueue on their own (issue #66). Still: no invite reaches a calendar — the schedule confirmation carries a link to the `.ics`, not an attached invitation — and while credential-gated live adapters exist, none has met a real API | [`apps/web/src/communications/ComposePanel.tsx`](../../apps/web/src/communications/ComposePanel.tsx) | `PRD-COM-001`, `PRD-SPK-002`, `JNY-004`, `JNY-009`, `ACC-INTEGRATION`, `GAP-010` |
...
| 7 | "native, one-way integration with Accelevents (our existing registration platform) to eliminate manual data re-entry" (issue #58) | **partial** — an outbound HTTP client with a field mapping and idempotency key exists behind the credential-gated `live` switch and passes the shared provider contract suite against a stubbed `fetch`. It has never exchanged a request with the real API, and there is still no organizer surface: no mapping configuration, connection test, dry run or visible sync state | [`apps/api/src/adapters/providers/accelevents-provider.ts`](../../apps/api/src/adapters/providers/accelevents-provider.ts) | `PRD-INT-001`, `JNY-009`, `ACC-INTEGRATION`, `GAP-012` |
