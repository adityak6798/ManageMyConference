...
Status: canonical | Owner: product | Evidence: `EVD-001`, `EVD-002`, `EVD-003` | Last verified: 2026-08-12 (working tree: commit `bb637d4`)
...
| 3 | templated speaker communications, and "calendar invites delivered directly to each speaker's own calendar (Gmail, Outlook, iCal)" (issues #52, #56) | **partial** — immutable templates, a durable outbox with retry/terminal/recovery, a conformant `.ics` download, and an organizer who can compose a template version and send it to the event's speakers. The calendar half now has all three routes the brief names: an iTIP `METHOD:REQUEST` invitation with `ORGANIZER` and `ATTENDEE` that an organizer sends per speaker per session through the outbox (idempotent on an unchanged agenda, a higher `SEQUENCE` when a session moves), Google and Outlook links per session in the portal, and the existing `.ics` for Apple Calendar. Still: nothing is triggered by a lifecycle event, and **no mail client has ever rendered one of these invitations** — the fixture provider sends no mail, so what is proven is that the invitation is built correctly and reaches the provider, not that Gmail showed an Accept button | [`apps/api/src/application/content/calendar-invite.ts`](../../apps/api/src/application/content/calendar-invite.ts) | `PRD-COM-001`, `PRD-SPK-002`, `JNY-004`, `JNY-009`, `ACC-INTEGRATION`, `GAP-010` |
