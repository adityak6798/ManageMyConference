...
- `GAP-003` GitHub branch-protection settings and successful execution of the configured gitleaks job are external to the repository and have not been verified from a run artifact. This is an operational-verification gap, not a failure of locally testable `ACC-HARNESS` behavior. Future product release gates still need the competition evaluator, accessibility/performance checks, deployment preview/smoke/rollback, and live provider contracts once those systems exist. Owner: platform. Governing ID: `ENG-CI-001`. Closure: inspect a successful required-check run, verify protection settings, and add each future release gate before its corresponding capability ships.
