...
| `ACC-SPEAKER` | `JNY-004`, `JNY-005` | portal/task/asset/calendar E2E | passed locally 2026-08-11: unit, API, D1 persistence, role-negative, deterministic upload-port/calendar, and Playwright journey evidence complete. Asset read paths were added this pass with an authorization matrix test at both the service and transport layers, but publishing an asset is one-way and ignores event publication state (#86), and #62 stays open until that lifecycle is complete. ICS still omits `DTSTAMP` (#75) |
