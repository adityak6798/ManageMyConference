...
Migration `0003_review_workflow.sql` introduces CFP-owned submissions/configured statuses/status audit and review-owned plans, assignments, conflicts, evaluations, outcomes, and completion-event records. Migrations `0004_review_completion_conflict_guard.sql` and `0005_review_conflict_completion_guard.sql` enforce conflict/completion mutual exclusion under concurrent requests. Migrations `0006_review_assignment_requires_plan.sql` and `0007_review_plan_lock.sql` atomically require a plan before assignment and lock rubric changes after assignment. Migrations `0008_cfp_transition_status_guard.sql` and `0009_cfp_status_in_use_guard.sql` atomically keep proposal transitions and configured-status edits consistent. Review reads proposal content only through the submitted-proposal application interface. The deterministic reset includes organizer triage, multi-role capability, and reviewer assignment perspectives.
