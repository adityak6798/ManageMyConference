...
Content routes are defined by the generated OpenAPI contract. `POST /api/events/{eventId}/content/accept` atomically consumes a proposal/review outcome without reading CFP or review storage and is idempotent by event/proposal provenance. `GET /api/events/{eventId}/content` returns an organizer-wide or person-scoped speaker workspace. Organizer-only task/message commands request speaker work and record communication; speaker-only completion, profile, and upload mutations enforce the event/person boundary before writes. R2 stores canonical asset bytes while metadata distinguishes `private` from explicitly `publishable` assets. `GET /api/events/{eventId}/speaker-calendar.ics` derives deterministic calendar bytes from canonical content schedule references.
