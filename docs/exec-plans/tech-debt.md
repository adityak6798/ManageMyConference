...
| `DEBT-010` | CFP draft editing has no optimistic concurrency, so two organizers editing one form silently overwrite each other | cfp | `PRD-CFP-001` | A stale write is refused with a typed conflict the composer can explain | #20 |
