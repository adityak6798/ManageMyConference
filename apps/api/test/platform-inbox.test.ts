  ...
  return {
    ...
    agenda: {
      draft: async () => ({
        ...
        occurrences: { sessions: { "session-1": 1 }, resources: 1 },
        conflicts: [],
      }),
    },
    publishing: {
...
describe("the operational inbox", () => {
  ...
  it("keeps a programme dismissal across an edit to a different session", async () => {
    ...
    const withTwo = (occurrences: { sessions: Record<string, number>; resources: number }) => ({
      agenda: {
        draft: async () => ({
          rooms: [{ id: "room-main", name: "Main stage" }],
          slots: [
    ...
    const dismissals = new MemoryInboxDismissalStore();
    const inboxWith = (occurrences: { sessions: Record<string, number>; resources: number }) =>
    ...
    const before = itemsOf(
      await inboxWith({ sessions: {}, resources: 0 }).inbox(organizer, EVENT_ONE),
      "programme",
    );
    const dismissed = before.find(({ title }) => title === "Accessible by default")?.key ?? "";
    await inboxWith({ sessions: {}, resources: 0 }).dismiss(organizer, EVENT_ONE, dismissed);

    // The organizer places and unplaces the *other* session, twice, and edits the resources.
    const after = itemsOf(
      await inboxWith({ sessions: { "session-1": 7 }, resources: 9 }).inbox(organizer, EVENT_ONE),
      "programme",
    );
    ...
    const clash = (occurrence: number) => ({
      agenda: {
        draft: async () => ({
          ...
          occurrences: { sessions: { "session-1": occurrence }, resources: 1 },
          conflicts: [
            {
              kind: "ROOM_OVERLAP",
              placementId: "placement-1",
