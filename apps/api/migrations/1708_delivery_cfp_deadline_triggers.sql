...
--
-- Migration `1706` already admitted `reviewer.reminder`; this rebuild preserves that mainline
-- value while adding both CFP values. Both are excluded from `REQUESTABLE_TRIGGERS` and from
-- `requestTriggerTypeSchema`. The
...
CREATE TABLE communication_deliveries_next (
  ...
  trigger_type TEXT NOT NULL CHECK (trigger_type IN ('speaker.invited', 'reviewer.assigned', 'reviewer.reminder', 'organizer.digest', 'projection.requested', 'schedule.published', 'speaker.scheduled', 'speaker.task_assigned', 'speaker.task_reminder', 'speaker.calendar_invite', 'decision.recorded', 'proposal.submitted', 'cfp.deadline_approaching', 'cfp.call_closed')),
