...
-- `code_verifier`, no `state_proof`, no session token, no cookie value. `detail` is for the shape
-- of an action, never for the secret that authorized it.
--
...
-- them -- actor, source, action, subject, organization, event, time, correlation -- and this lane
...
CREATE TABLE identity_audit_events (
  ...
  -- is a table rebuild, so the identity lane's whole vocabulary is declared here: durable
  ...
  -- What kind of caller acted. #99's vocabulary also carries `agent`; identity-access has no
  -- agent-initiated action today, and admitting a value nothing writes would be a claim this
  -- table cannot support, so the column is the three that occur. Widening it is a rebuild, and
  -- belongs to whichever lane first has an agent actor.
  ...
  event_id       TEXT,
  -- Always present: this is what turns a report of "it refused me at 14:03" into the row.
