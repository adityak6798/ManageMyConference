...
-- cookie now carries the `sid` of its row, the resolver reads the row on every authenticated
...
CREATE TABLE identity_sessions (
  ...
  -- Whose session this is, and the only thing this column is for: it scopes revocation -- "end
  -- every session of this user" -- and is compared against the id the signed cookie already
  -- carries. It is never followed to produce an actor; see docs/architecture/authorization.md.
