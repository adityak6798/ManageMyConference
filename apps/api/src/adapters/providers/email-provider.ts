...
/**
 * The calendar part of an invitation, when the delivery carries one.
 *
 * An invitation reaches a speaker's calendar by arriving *as mail*: a message whose calendar body
 * part is `text/calendar; method=REQUEST` is what Gmail, Outlook and Apple Calendar turn into an
 * Accept/Decline card that writes to the calendar. A link or a plain file attachment does not do
 * that, which is why this is a distinct field rather than an attachment among others — the
 * provider is expected to render it as an alternative part carrying the method parameter, not as
 * a download.
 *
 * It travels on the payload rather than in a column because it is *content*, generated by the
 * content domain from session and speaker data (`buildSpeakerInvite`) and stored with the
 * delivery so a retry sends the invitation that was composed rather than one rebuilt from a
 * schedule that has since moved.
 *
 * A delivery with no `calendarInvite` produces exactly the request an email delivery produced
 * before this existed.
 */
const CALENDAR_METHODS = new Set(["REQUEST", "CANCEL"]);
...
const calendarPartOf = (
  payload: Readonly<Record<string, unknown>>,
): CalendarPart | null | false => {
  const candidate = payload.calendarInvite;
  if (candidate === undefined || candidate === null) return null;
  if (typeof candidate !== "object") return false;
  const { method, filename, content } = candidate as Record<string, unknown>;
  if (typeof content !== "string" || !content) return false;
  if (typeof method !== "string" || !CALENDAR_METHODS.has(method)) return false;
  return {
    method,
    filename: typeof filename === "string" && filename ? filename : "invite.ics",
    content,
  };
};
...
export class HttpEmailProvider implements DeliveryProvider {
  ...
  async deliver(delivery: Delivery): Promise<ProviderResult> {
    ...
    const calendar = calendarPartOf(delivery.payload);
    // A delivery that was supposed to carry an invitation and carries something unreadable is
    // terminal rather than sent without it: a speaker who receives the covering note and no
    // invitation has been told a meeting exists and given no way to accept it, and no number of
    // retries will repair a payload that is already stored.
    if (calendar === false) return { kind: "terminal", code: "CALENDAR_INVITE_MALFORMED" };
    ...
    try {
      response = await this.fetch(this.configuration.endpoint, {
        ...
        body: JSON.stringify({
          ...
          // Absent, not null, when there is no invitation — so the request an ordinary email
          // delivery produces is byte-identical to the one produced before invitations existed.
          ...(calendar ? { calendar } : {}),
        }),
